...
//        cerr << "spawning at " << currentPosition.first << " " << currentPosition.second << endl;
        ...
        enemies.push_back(new Enemy(curW, curH, currentPosition.first, currentPosition.second, curSpeed, curAngle, canSpilt, curHP, DIRS[indexEnemy]));
    }
    for (int i = 0; i < int(enemies.size()); i++) {
        ...
        if (enemies[i]->health_point <= EPS) {
            if (enemies[i]->canSpilt) {
                for (int j = 0; j < 4; j++) {
                    ...
                    if (j == 2) nextY -= 45;
                    if (j == 3) nextY += 45;
//                    cerr << "push" << nextX << " " << nextY << endl;
                    enemies.push_back(new Enemy(32, 32, nextX, nextY, randDouble(1.5, 2), enemies[i]->getAngle(), false, 1, DIRS[0]));
                }
            }
            killEnemy(i);
            score.update(1);
